#include "6-model.h"
#include "stb_image.h" // Ensure this is reachable
#include <iostream>
...
void Model::SetupInstancing(GLuint instanceVBO) {
  for (auto& m : meshes) {
    m.SetupInstancingAttributes(instanceVBO);
  }
}
...
void Model::Draw(common::Pipeline& pipeline, int instanceCount) {
  for (const auto& mesh : meshes) mesh.Draw(pipeline, instanceCount);
}
...
std::vector<Texture> Model::loadMaterialTextures(
    const aiMaterial* mat, aiTextureType type, std::string typeName) {
  std::vector<Texture> textures;
